...
export default function AdminProposalDashboard() {
    ...
    // Role check
    const isKetuaLppm = hasRole('admin') || hasPermission('proposals.approve');
    ...
    const handleLppmApproval = async () => {
        setIsApproving(true);
        try {
            await axios.post(`/api/admin_proposals/${approvalModal.proposal.id}/approve-lppm`, {
                status: approvalModal.status,
                note: approvalModal.note
            }, {
                headers: { Authorization: `Bearer ${token}` }
            });
            toast.success(`Usulan berhasil ${approvalModal.status === 'approved' ? 'disetujui' : 'ditolak'}!`);
    ...
    const handleBatchAssign = async () => {
        if (!confirm('Plot semua usulan baru secara otomatis ke reviewer tersedia?')) return;
        setIsLoading(true);
        try {
            const res = await axios.post('/api/admin_proposals/batch-assign', {}, {
                headers: { Authorization: `Bearer ${token}` }
            });
                    ...
                    <table className="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody className="bg-white divide-y divide-gray-100">
                            {isLoading ? (
                                ...
                                <tr><td colSpan={6} className="py-10 text-center text-sm text-gray-400">Tidak ada usulan yang ditemukan.</td></tr>
                        ...
                        <div className="bg-green-800 p-4 text-white flex justify-between items-center">
                            <div>
                                <h3 className="font-bold uppercase tracking-widest text-xs flex items-center gap-2">
                                    <ShieldCheck size={16} /> Persetujuan Ketua LPPM
                                </h3>
